...
try:
    ...
    get_train_loader = None
    split_into_groups = None
    ElementwiseLoss = None
    Loss = None
    MultiTaskLoss = None
from src.models import Discriminator
import multiprocessing as mp
from src.utils import *
from src.fediir import default_fediir_penalty, fediir_penalty
try:
    import wandb
except ImportError:
    class _NoWandb:
...
class ERM(object):
    ...
    def __init__(self, client_id, device, dataset, ds_bundle, hparam):
        ...
        if get_train_loader is None:
            raise ImportError("WILDS is required to initialize training clients. Install the requirements before running experiments.")
        ...
        self.save_checkpoints = bool(self.hparam.get("save_checkpoints", False))
        self.saved_optimizer = False
        run_id = self.hparam.get("id", "default")
        self.opt_dict_path = os.path.join(self.hparam['data_path'], "opt_dict", run_id, "client_{}.pt".format(self.client_id))
        self.sch_dict_path = os.path.join(self.hparam['data_path'], "sch_dict", run_id, "client_{}.pt".format(self.client_id))
        if self.save_checkpoints:
            os.makedirs(os.path.dirname(self.opt_dict_path), exist_ok=True)
            os.makedirs(os.path.dirname(self.sch_dict_path), exist_ok=True)
            if os.path.exists(self.opt_dict_path): os.remove(self.opt_dict_path)

    def setup_model(self, featurizer, classifier):
        self._featurizer = featurizer
        self._classifier = classifier
        self.featurizer = self._wrap_data_parallel(self._featurizer)
        self.classifier = self._wrap_data_parallel(self._classifier)
        self.model = self._wrap_data_parallel(nn.Sequential(self._featurizer, self._classifier))

    def _data_parallel_device_ids(self):
        device = torch.device(self.device)
        if device.type == "cuda" and device.index is not None:
            return [int(device.index)]
        return None

    def _wrap_data_parallel(self, module):
        device_ids = self._data_parallel_device_ids()
    ...
    def set_device(self, device):
        ...
        if getattr(self, "_generator", None) is not None:
            self.generator = self._wrap_data_parallel(self._generator)
        if getattr(self, "_discriminator", None) is not None:
            self.discriminator = self._wrap_data_parallel(self._discriminator)
    ...
    def init_train(self):
        ...
        if self.save_checkpoints and self.saved_optimizer:
    ...
    def fit(self, server_round):
        ...
        for e in range(self.local_epochs):
            for batch in self._tqdm(self.dataloader):
                ...
                wandb.log(
                    {
                        ...
                        "loss/{}".format(self.client_id): training_loss/len(self.dataset),
                    },
                    step=server_round*self.local_epochs,
                )
        self.end_train()
    ...
    def _stat_sketch_config(self, override=None):
        override = override or {}
        return SketchConfig(
            mode=override.get("mode", self.hparam.get("activation_sketch_mode", "diag")),
            max_batches=int(override.get("max_batches", self.hparam.get("activation_sketch_max_batches", 1))),
            max_patches=int(override.get("max_patches", self.hparam.get("activation_sketch_max_patches", 2048))),
            max_full_dim=int(override.get("max_full_dim", self.hparam.get("activation_sketch_max_full_dim", 4096))),
            block_size=int(override.get("block_size", self.hparam.get("activation_sketch_block_size", 512))),
            dtype=override.get("dtype", self.hparam.get("activation_sketch_dtype", "float32")),
            device=override.get("device", self.hparam.get("activation_sketch_device", "cpu")),
            shrinkage_alpha=float(override.get("shrinkage_alpha", self.hparam.get("activation_sketch_shrinkage_alpha", 0.75))),
            dp_epsilon=float(override.get("dp_epsilon", self.hparam.get("activation_sketch_dp_epsilon", -1.0))),
            dp_delta=float(override.get("dp_delta", self.hparam.get("activation_sketch_dp_delta", 1e-5))),
            dp_clip=float(override.get("dp_clip", self.hparam.get("activation_sketch_dp_clip", 0.0))),
            lowrank_rank=int(override.get("lowrank_rank", self.hparam.get("activation_sketch_lowrank_rank", 64))),
            random_projection_dim=int(override.get("random_projection_dim", self.hparam.get("activation_sketch_random_projection_dim", 256))),
            random_seed=int(override.get("random_seed", self.hparam.get("activation_sketch_random_seed", 0))),
        )

    def _forward_for_activation_sketch(self, batch):
        x = batch[0].to(self.device)
        return self.model(x)

    def collect_activation_sketches(self, model_state, layer_names=None, max_batches=None, sketch_config=None):
        """Collect activation Gram sketches from source data only.

        The payload contains only Gram statistics and counts. Raw batches and
        raw activations never leave this method.
        """
        cfg = self._stat_sketch_config(sketch_config)
        ...
        try:
            with torch.no_grad():
                with ActivationSketcher(self.model, cfg, layer_names=layer_names) as sketcher:
                    for batch_idx, batch in enumerate(self.dataloader):
                        if batch_idx >= cfg.max_batches:
                            break
                        _ = self._forward_for_activation_sketch(batch)
    ...
    def compute_diagonal_fisher(
        ...
        max_batches=1,
        fisher_eps=1e-8,
        label_mode="true_labels",
        fisher_clip=0.0,
        fisher_normalize=False,
    ):
        """Compute a diagonal Fisher approximation on source data only."""
        was_training = self.model.training
        self.model.load_state_dict(model_state)
        self.model.to(self.device)
        self.model.train()
        ...
        try:
            for batch_idx, batch in enumerate(self.dataloader):
                if batch_idx >= int(max_batches):
                    break
                x, y_true = batch[0].to(self.device), batch[1].to(self.device)
                self.model.zero_grad(set_to_none=True)
                y_pred = self.model(x)
                if label_mode == "predicted_labels":
                    y_used = torch.argmax(y_pred.detach(), dim=-1)
                else:
                    y_used = y_true
                loss = self.ds_bundle.loss.compute(y_pred, y_used, return_dict=False).mean()
                loss.backward()
                batch_size = int(x.shape[0])
                total += batch_size
                for name, param in self.model.named_parameters():
                    if param.grad is None or name not in fisher:
                        continue
                    grad_sq = param.grad.detach().cpu().float().pow(2)
                    if fisher_clip and float(fisher_clip) > 0:
                        grad_sq = grad_sq.clamp(max=float(fisher_clip))
                    fisher[name] += grad_sq * batch_size
            denom = max(total, 1)
            for name in list(fisher.keys()):
                fisher[name] = fisher[name] / denom
                if fisher_normalize:
                    fisher[name] = fisher[name] / (fisher[name].mean().clamp_min(float(fisher_eps)))
        finally:
            self.model.zero_grad(set_to_none=True)
            if was_training:
                self.model.train()
    ...
    def compute_gaussian_summary(self, model_state=None, stat_source="rgb", max_batches=1):
        """Summarize source-domain RGB tensors or penultimate features as diagonal Gaussians."""
        stat_source = str(stat_source).lower()
        was_training = self.model.training
        ...
        try:
            with torch.no_grad():
                for batch_idx, batch in enumerate(self.dataloader):
                    ...
                    if stat_source == "rgb":
                        if x.dim() == 4:
                            values = x.permute(1, 0, 2, 3).reshape(x.shape[1], -1).T
                        else:
                            values = x.reshape(x.shape[0], -1)
                    elif stat_source == "features":
                        features = self.featurizer(x)
                        if getattr(self._featurizer, "probabilistic", False):
                            z_dim = int(features.shape[-1] / 2)
                            features = features[..., :z_dim]
                        values = features.reshape(features.shape[0], -1)
                    else:
                        raise ValueError("stat_source must be 'rgb' or 'features'")
                    values = values.detach().float()
                    batch_sum = values.sum(dim=0)
                    batch_sum2 = values.pow(2).sum(dim=0)
        ...
        if count <= 0:
            return {}
        mean = (sum_x / count).detach().cpu()
        var = (sum_x2 / count - mean.to(sum_x2.device).pow(2)).clamp_min(0).detach().cpu()
        return {
            "count": int(count),
            "mean": mean,
            "var": var,
...
class FedIIR(ERM):
    """FedIIR client update from Guo et al., ICML 2023.

    The server or station must compute and broadcast the EMA-smoothed mean
    classifier gradient over the sampled clients before local training.
    """

    def __init__(self, client_id, device, dataset, ds_bundle, hparam):
        super().__init__(client_id, device, dataset, ds_bundle, hparam)
        penalty = hparam.get("fediir_penalty", None)
        if penalty is None:
            penalty = default_fediir_penalty(hparam.get("dataset", ds_bundle.name))
        self.fediir_penalty_weight = float(penalty)
        self.fediir_grad_mean = None
        self._last_fediir_penalty = 0.0

    def _classifier_parameters(self):
        return tuple(self.classifier.parameters())

    def set_fediir_grad_mean(self, grad_mean):
        self.fediir_grad_mean = tuple(grad.detach().cpu().clone() for grad in grad_mean)

    def compute_fediir_classifier_grad_sum(self, max_batches=None):
        """Return sum of classifier gradients and number of batches.

        This mirrors the official FedIIR `mean_grad` loop: gradients are
        computed on the current global model, for classifier parameters only,
        and averaged by number of local batches rather than number of samples.
        """
        was_training = self.model.training
        self.model.train()
        self.model.to(self.device)
        ...
        try:
            for batch_idx, batch in enumerate(self.dataloader):
                ...
                self.model.zero_grad(set_to_none=True)
                logits = self.model(x)
                loss = self.ds_bundle.loss.compute(logits, y_true, return_dict=False).mean()
                ...
                grad_cpu = tuple(grad.detach().cpu() for grad in grad_batch)
                if grad_sum is None:
                    grad_sum = tuple(torch.zeros_like(grad) for grad in grad_cpu)
                grad_sum = tuple(total + grad for total, grad in zip(grad_sum, grad_cpu))
                total_batches += 1
        finally:
            self.model.zero_grad(set_to_none=True)
            if was_training:
            ...
            if self.device == "cuda":
                torch.cuda.empty_cache()
        if grad_sum is None:
            grad_sum = tuple(torch.zeros_like(param.detach().cpu()) for param in self._classifier_parameters())
        return grad_sum, total_batches

    def fit(self, server_round):
        if self.fediir_grad_mean is None:
            raise RuntimeError(
                "FedIIR requires a server/station to call set_fediir_grad_mean() "
                "before local training. Use server_method='FedIIRServer' for flat "
                "FedIIR, or HierarchicalFedAvg/HFedATM with client_method='FedIIR'."
            )
        self.init_train()
        training_loss = 0.0
        ...
        for e in range(self.local_epochs):
            for batch in self._tqdm(self.dataloader):
                results = self.process_batch(batch)
                training_loss += self.step(results)
                training_penalty += self._last_fediir_penalty
            if self.hparam['wandb']:
                wandb.log(
                    {
                        "comm_round": server_round,
                        "loss/{}".format(self.client_id): training_loss / len(self.dataset),
                        "fediir/penalty/{}".format(self.client_id): training_penalty / max(len(self.dataloader), 1),
                    },
                    step=server_round * self.local_epochs,
                )
        self.end_train()
    ...
    def step(self, results):
        ...
        penalty_value = fediir_penalty(grad_client, self.fediir_grad_mean)
        objective = loss_erm + self.fediir_penalty_weight * penalty_value
        objective.backward()
        self.optimizer.step()
        self.optimizer.zero_grad()
        self._last_fediir_penalty = float(penalty_value.detach().cpu())
        return total_loss


class MTGCClient(ERM):
    """Client update for exact MTGC linear gradient-correction objective."""

    def __init__(self, client_id, device, dataset, ds_bundle, hparam):
        super().__init__(client_id, device, dataset, ds_bundle, hparam)
        self.mtgc_correction_vector = None

    def set_mtgc_correction_vector(self, correction_vector):
        self.mtgc_correction_vector = correction_vector.detach().cpu().float().clone()

    def _flat_parameter_vector(self):
        return torch.cat([
            param.reshape(-1)
    ...
    def fit(self, server_round):
        if self.mtgc_correction_vector is None:
            raise RuntimeError(
                "MTGCClient requires MTGC server to call set_mtgc_correction_vector() "
                "before local training."
            )
        super().fit(server_round)

    def step(self, results):
        loss = self.ds_bundle.loss.compute(results['y_pred'], results['y_true'], return_dict=False)
        objective = loss.mean()
        total_loss = loss.sum().item()
        correction = self.mtgc_correction_vector.to(self.device)
        flat_params = self._flat_parameter_vector()
        if flat_params.numel() != correction.numel():
            raise RuntimeError(
                "MTGC correction vector length {} does not match parameter vector length {}".format(
                    correction.numel(),
                    flat_params.numel(),
                )
            )
        objective = objective + torch.sum(flat_params * correction)
        objective.backward()
        self.optimizer.step()
        self.optimizer.zero_grad()
        return total_loss
        ...
        for e in range(self.local_epochs):
            ...
            if self.hparam['wandb']:
                wandb.log(
                    {
                        "comm_round": server_round,
                        "aln_loss/{}".format(self.client_id): training_loss/len(self.dataset),
                    },
                    step=server_round*self.local_epochs,
                )
        ...
        for e in range(self.second_local_epochs):
            ...
            if self.hparam['wandb']:
                wandb.log(
                    {
                        "comm_round": server_round,
                        "cla_loss/{}".format(self.client_id): training_loss[0]/len(self.dataset),
                        "dist_loss/{}".format(self.client_id): training_loss[1]/len(self.dataset),
                        "gen_loss/{}".format(self.client_id): training_loss[2]/len(self.dataset),
                    },
                    step=server_round*self.local_epochs,
                )
        self.end_train()
...
class FedSR(ERM):
    def __init__(self, client_id, device, dataset, ds_bundle, hparam): 
        ...
        ref_dir = hparam.get("fedsr_ref_dir")
        if ref_dir is None:
            run_id = str(hparam.get("id", "default_run"))
            ref_dir = os.path.join(hparam.get("data_path", "./log"), "tmp_fedsr", run_id)
        os.makedirs(ref_dir, exist_ok=True)
        self.fp = os.path.join(ref_dir, "fedsr_ref_client_{}.pt".format(self.client_id))
    ...
    def _forward_for_activation_sketch(self, batch):
        return self.process_batch(batch)["y_pred"]
