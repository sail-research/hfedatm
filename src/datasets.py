...
class CachedPACS(PACS):
    def __init__(
        ...
        self._cache_dir = Path(cache_dir) if cache_dir else self.data_dir / "tensor_cache_224_float32_imagenet"
        self._cache_file = self._cache_dir / "images.npy"
        self._cache_meta_file = self._cache_dir / "metadata.json"
        if not self._cache_file.exists():
            raise FileNotFoundError(
                "PACS tensor cache is missing at {}. Build it with "
                "`python scripts/build_pacs_tensor_cache.py --root_dir <dataset_path>`.".format(
                    self._cache_file
                )
            )
        self._tensor_cache = np.load(self._cache_file, mmap_mode="r")
        if self._tensor_cache.shape[0] != len(self._input_array):
            raise ValueError(
                "PACS tensor cache has {} examples, but metadata has {}.".format(
                    self._tensor_cache.shape[0], len(self._input_array)
                )
            )
        if self._tensor_cache.shape[1:] != (3, 224, 224):
            raise ValueError(
                "Expected PACS tensor cache shape N x 3 x 224 x 224, got {}.".format(
                    self._tensor_cache.shape
                )
            )
        if self._cache_meta_file.exists():
            with open(self._cache_meta_file) as f:
                self._tensor_cache_metadata = json.load(f)
        else:
            self._tensor_cache_metadata = {}

    def get_input(self, idx) -> torch.Tensor:
        return torch.from_numpy(self._tensor_cache[idx])
