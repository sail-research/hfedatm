...
class LeNet5(nn.Module):
    """LeNet-5 style featurizer for image DG experiments.

    The original LeNet-5 assumes 32x32 inputs. We keep that path exact for
    PACS configs that set image_size=32, while using adaptive pooling as a
    shape guard for other image sizes.
    """
    def __init__(self, input_shape, probabilistic=False):
        super(LeNet5, self).__init__()
        self.probabilistic = probabilistic
        self.n_outputs = 84
        out_features = self.n_outputs * 2 if probabilistic else self.n_outputs
