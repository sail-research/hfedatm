...
from .models import ResNet, code_gpt_py, Classifier, DistilBertFeaturizer, CNN, DenseNet, LeNet5
...
class ObjBundle(object):
    def __init__(self, dataset, probabilistic=False) -> None:
        ...
        self.backbone = getattr(dataset, "_hfed_backbone", "resnet50")
    ...
    def _build_featurizer(self):
        ...
        if backbone in {"resnet", "resnet50"}:
            return ResNet(self.input_shape, probabilistic=self.probabilistic)
        if backbone in {"lenet", "lenet5", "lenet-5"}:
            return LeNet5(self.input_shape, probabilistic=self.probabilistic)
        if backbone in {"cnn", "smallcnn", "small_cnn"}:
            return CNN(self.input_shape, probabilistic=self.probabilistic)
        raise ValueError(f"Unsupported image backbone: {self.backbone}")
...
class PACS(ObjBundle):
    ...
    @property
    def _input_shape(self):
        image_size = int(getattr(self.dataset, "_hfed_image_size", 224))
