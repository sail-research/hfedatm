import os
from src.dataset_bundle import CelebA as CelebABundle
import torch
import copy
import time
...
def worker(root_dir, queue):
    ...
    ds = CelebABundle(dataset, feature_dimension=1024)
    # dataset._metadata_array = new_metadata_array
...
def listener(root_dir, process_num, queue):
    root_dir = root_dir / Path("celebA_v1.0/img_align_celeba/")
    while True:
        obj = queue.get()
